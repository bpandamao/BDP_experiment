...
### the K
 ...
 for(i in 1:n){
   ...
   for (g in 1:K){
    ...
    s1=s1+dnorm(Y[i],t(G.s)%*%beta_dens.s,sqrt(8*kc[i]*sigma),log=TRUE)
    s2=s2+dnorm(Y[i],t(G)%*%beta_dens,sqrt(8*kc[i]*sigma),log=TRUE)
 }
    ...
    beta_dens.s=NULL
      ...
      for (jj in 1:K){
        beta_dens.s[jj]=dbeta(w.s,jj,K-jj+1)
 			...
 			s1=s1+dnorm(Y[i],t(G)%*%beta_dens.s,sqrt(8*kc[i]*sigma),log=TRUE)
 			s2=s2+dnorm(Y[i],t(G)%*%Beta_dens[,i],sqrt(8*kc[i]*sigma),log=TRUE)
 			Beta_dens.s[,i]=beta_dens.s
 		}   
...
### the V
 for(l in sample(1:L,L)){
 	...
 	s1=s2=0
  for(i in 1:n){
 		s1=s1+dnorm(Y[i],t(G.s)%*%Beta_dens[,i],sqrt(8*kc[i]*sigma),log=TRUE)
 		s2=s2+dnorm(Y[i],t(G)%*%Beta_dens[,i],sqrt(8*kc[i]*sigma),log=TRUE)
 	}
 ...
 eta2=2/sigma
  for (i in 1:n){
    eta1=((Y[i]-t(G)%*%Beta_dens[,i])^2)/(8*sigma)
